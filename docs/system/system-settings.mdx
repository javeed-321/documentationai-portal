...
Several of the timer fields below are covered in more detail under [Timers and Options](/docs/destinations/timers-options).
...
| Open Time | Enter the number of seconds OM Plus will use to try and open a connection to a network device. See [Timers and Options](/docs/destinations/timers-options). |
| Write Time | Enter the number of seconds OM Plus requires to try to write data to a destination before marking the request interrupted and disabling the destination. A successful write to the destination resets this timer. See [Timers and Options](/docs/destinations/timers-options). |
| Purge Delay | Enter the amount of time in seconds to keep a printed request before purging or archiving it. It has a value range of 0-99999. See [Timers and Options](/docs/destinations/timers-options). |
| Close Delay | Enter the number of seconds OM Plus holds the connection to this device open to allow it to flush its buffers. See [Timers and Options](/docs/destinations/timers-options). |
| Retry Time | Number of seconds allowed between attempts to deliver a job. See [Timers and Options](/docs/destinations/timers-options). |
